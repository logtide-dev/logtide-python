...
def logtide_json_dumps(obj: Any) -> str:
    try:
        return json.dumps(
            obj,
            default=lambda o: to_json_value(o, set()),
            separators=(",", ":"),
            ensure_ascii=False,
        )
    except Exception:
...
def to_json_value(o: Any, seen: set[int]) -> JsonValue:
    try:
        ...
        if isinstance(o, (list, tuple)):
        ...
        elif isinstance(o, Mapping):  # eg: dict, smth that is like dict
            return {
            ...
            return {
                f.name: to_json_value(getattr(o, f.name), seen)
...
def _numpy_array_encoder(o: Any, seen: set[int]) -> JsonValue:
    ...
    for dimension in range(dimensions):
        if shape[dimension] <= NUMPY_DIMENSION_MAX_SIZE:
            continue
        # In case of multi-dimensional, we limit the dimension size by the NUMPY_DIMENSION_MAX_SIZE.
